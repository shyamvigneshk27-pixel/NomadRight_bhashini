...
# UIState (see boards/hdmi.py) sends one of these. Besides the screen text
# fields, two patches carry structured data:
#   {"answer": {"native": ..., "en": ..., "label": ...} | null} - the last
#       answer in the worker's language (boards/hdmi.py's answer_text()),
#   {"asr_languages": ["hi", "ta", ...]} - languages the device can hear.
MSG_STATE_PATCH = "state_patch"

# Plain HTTP (not WebSocket): GET /api/schemes returns the scheme catalogue
# for the Schemes page - {"en": [...], "hi": [...], "ta": [...]} generated by
# scheme_intel/build_catalog.py from the knowledge base (see server.py).
